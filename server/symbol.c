...
// tmp
#define pr_dbg(s, args...)
#define pr_dbg2(s, args...)
#define pr_dbg3(s, args...)
#define pr_err(s, args...)
#define pr_out(s, args...)
		       ...
		       unsigned long offset, unsigned long flags)
{
	...
	symtab->sym = realloc(symtab->sym, symtab->nr_sym * sizeof(*symtab->sym));
...
static void sort_dynsymtab(struct symtab *dsymtab)
{
	 ...
	 */
	dsymtab->sym_names = malloc(sizeof(*dsymtab->sym_names) * dsymtab->nr_sym);
		  ...
		  const char *filename)
{
	...
	if (dirname != NULL && (symtabs->flags & SYMTAB_FL_USE_SYMFILE)) {
		...
		asprintf(&symfile, "%s/%s.sym", dirname, basename(filename));
		     ...
		     unsigned long offset)
{
	...
	stab->sym_names = malloc(sizeof(*stab->sym_names) * stab->nr_sym);
		      ...
		      const char *exename)
{
	...
	asprintf(&symfile, "%s/%s.sym", dirname, basename(exename));
	...
	for (i = 0; i < dsymtab->nr_sym; i++) {
		for (k = 0; k < symcount; k++) {
			if (!strcmp(dsymtab->sym_names[i]->name, symlist[k])) {
				idx = realloc(idx, (count + 1) * sizeof(*idx));
	...
	if (maps) {
		if (maps->symtab.nr_sym == 0) {
			...
			if (symtabs->flags & SYMTAB_FL_USE_SYMFILE) {
				...
				asprintf(&symfile, "%s/%s.sym", symtabs->dirname,
